...
#linear加了一个dropout
#可以采样子网，采样的时候设置是否保留权重
#infeatures是list，outfeatures不是
...
#in_channel_list，out_channel_list，kernel_size_list，expand_ratio_list都是list
#就是MobileNetV2的逆线性瓶颈层
class DynamicMBConvLayer(MyModule):
    ...
    def forward(self, x):
        ...
        self.depth_conv.conv.active_kernel_size = self.active_kernel_size#depth_conv进行kernel_size动态设置
    ...
    #弹性宽度
    #在论文中提到了一点是需要对权重进行L1重要性的选取。代码中，主要通过每次PS训练前使用
    # re_organize_middle_weights(self, expand_ratio_stage=0)函数来对权重进行排序，排序完成后，在训练过程中，
    # 只需要按顺序self.conv.weight[:out_channel, :in_channel, :, :]正常截取即可。
    ...
    def re_organize_middle_weights(self, expand_ratio_stage=0):
        ...
        adjust_bn_according_to_idx(self.depth_conv.bn.bn, sorted_idx)#adjust_bn_according_to_idx用于调整bn权重顺序
...
#in_channel_list,out_channel_list是1list
#就是一个普通的卷积层
...
#in_channel_list,out_channel_list和expand_ratio_list是list
#其实就是逆残差线性瓶颈层，比上面那个多了一个残差，但是这里的kernael size时不变的,用的也不是relu6
class DynamicResNetBottleneckBlock(MyModule):
        ...
        self.conv1 = nn.Sequential(
            OrderedDict(
                [
                    (
                        "conv",
                        DynamicConv2d(max(self.in_channel_list), max_middle_channel, kernel_size, stride),
                    ),
                    ("bn", DynamicBatchNorm2d(max_middle_channel)),
                    ("act", build_activation(self.act_func, inplace=True)),
                ]
        ...
        self.conv2 = nn.Sequential(
            OrderedDict(
                [
                    (
                        "conv",
                        DynamicConv2d(max_middle_channel, max(self.out_channel_list),kernel_size),
                    ),
                    ("bn", DynamicBatchNorm2d(max(self.out_channel_list))),
                ]
            )
        )
        #决定使用conv还是avgpool_conv进行下采样
    ...
    def get_active_subnet(self, in_channel, preserve_weight=True):
        ...
        # copy weight from current layer
        data1=self.conv1.conv.get_active_filter(
    ...
    def re_organize_middle_weights_r(self, expand_ratio_stage=0):
        ...
        # )
        # adjust_bn_according_to_idx(self.conv2.bn.bn, sorted_idx)#adjust_bn_according_to_idx用于调整bn权重顺序
        ...
        if expand_ratio_stage > 0:
            ...
            for i in range(expand_ratio_stage + 1):
                ...
                base += 1e5
                right = left
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=False)
...
if __name__ == "__main__":
    x=[10,20,30]
    y = [10,20,400]
    layer1=DynamicResNetBottleneckBlock(x,y)
    # layer2=DynamicMBConvLayer(x,y)
    print(layer1.re_organize_middle_weights())
