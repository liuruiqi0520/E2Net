...
class DynamicSeparableConv2d(nn.Module):
    ...
    #弹性卷积核大小
    #对于弹性卷积核大小，OFA多设置了一个可学习的Transform矩阵，来弥补直接从大卷积核抠出一个小卷积核带来的性能损失。
    # 利用Transform矩阵对一个小卷积核做一个线性变换后再进行训练。并且转换的过程也是逐步进行的。比如要抠出一个3x3卷积核，
    # 就从7x7卷积中抠出5x5，做个变换。再从5x5抠出3x3，再做个变换，得到最终的3x3卷积核。

    #Transform矩阵的大小就是(HW)^2，即9x9（5->3）和25x25(7->5)，
    # 初始化方式为scale_params['%s_matrix' % param_name] = Parameter(torch.eye(ks_small ** 2))，
    # 即生成对角线全1，其余部分全0的二维数组来进行初始化。
...
#     x=[10,20]
#     y = [10,20,15]
#     layer1=DynamicSeparableConv2d(10,y)
#     print(layer1.get_active_filter(10,5))