...
import os
import numpy as np
from e2n_utils import self_search,set_stage
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    print(dataset.N_TASKS)
    st = time.time()
    finconfig_list = dict()
    finconfig_list["config"]=[]
    finconfig_list["loss"] = []
    for t in range(dataset.N_TASKS):
        # print(args.n_epochs)
        # args.n_epochs = 1
        ...
        if t:
            if model.NAME == 'e2n':
                last_config = self_search(t, model, sub_net_dict, last_config, finconfig_list, total_config_dict)
        ...
        for epoch in range(args.n_epochs):
            for i, data in enumerate(train_loader):
                ...
                if hasattr(dataset.train_loader.dataset, 'logits'):
                    ...
                    not_aug_inputs = not_aug_inputs.to(model.device)
                    if model.NAME == 'e2n':
                        loss = model.observe(inputs, labels, not_aug_inputs,sub_net_dict, config_list=last_config)
                    else:
        ...
        total_config_dict["class"] = round(mean_acc_class_il,2)
        total_config_dict["task"] = round(mean_acc_task_il, 2)
    ...
    save_json = True
    name_str = "class_" + str(total_config_list[dataset.N_TASKS-1]["class"]) + "_task_" + str(total_config_list[dataset.N_TASKS-1]["task"])
    save_path = "result/json/" + name_str + ".json"
    ed = time.time()
    print(round(ed-st,3))
    total_config_list["time"] = round(ed-st,3)
